document.addEventListener("DOMContentLoaded", () => {
    ...
    //remove tags generated by google translate
    function removeFontTags(node) {
        node.querySelectorAll("font").forEach(el => {
            el.replaceWith(...el.childNodes); // Înlocuiește tagul <font> cu conținutul său
        });
    }

    // Setează un observer care detectează modificările DOM set observer to detect the changes in DOM
    const observer = new MutationObserver(mutations => {
        mutations.forEach(mutation => {
            mutation.addedNodes.forEach(node => {
                if (node.nodeType === 1) { // Check if the element exists in HTML
                    removeFontTags(node);
                }
            });
        });
